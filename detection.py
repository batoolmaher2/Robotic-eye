...
class ObjectDetector:
    def __init__(self, model_name='yolov5s.pt', confidence_threshold=0.5):
        ...
        # Camera setup (use robust helper to support macOS AVFoundation and other backends)
        self.camera = open_camera(width=CAMERA_CONFIG.get('WIDTH', 640),
                                  height=CAMERA_CONFIG.get('HEIGHT', 480),
                                  fps=CAMERA_CONFIG.get('FPS', 30))

        # Verify camera connection
        if self.camera is None or not getattr(self.camera, 'isOpened', lambda: False)():
            raise RuntimeError("Failed to open camera. Check connection and permissions.")
